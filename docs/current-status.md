...
- Stronger model foundations now include packaged local profiles (`local-default`, `host-ollama`, `compose-ollama`, `vllm-gpu`), Ollama and vLLM-compatible embedding/answer-generation paths, public OpenAI-compatible embedding/answer adapters behind `PUBLIC_LLM_ENABLED=true`, model-status checks for embedding/answer/vector/reranker runtimes, deterministic local keyword reranking, HTTP-backed cross-encoder/vLLM reranking, latency warning threshold config, and UI surfaces for active model, profile, vector, reranker, and threshold state.
...
No active roadmap implementation slices remain in this POC scope.

Future production-specific work should be driven by the selected deployment target, approved public model names, rate limits, cost controls, and security review. Public token-based providers are available through the OpenAI-compatible adapter, but they remain disabled by default and require explicit `PUBLIC_LLM_ENABLED=true`, `PUBLIC_LLM_API_KEY`, and model-name configuration.
