...
- React/Vite UI: PKCE login/logout, document upload, mounted-path ingestion, queued upload status, job cancel/retry controls, read-only A&A and session status display, model runtime/profile readiness, evaluation quality gate display, admin analytics and recent operations summary, format guidance, document inventory/detail/chunk preview with extraction warnings, query form, citations, cache status, and latency display.
...
- Redis: query cache plus normal, OCR-heavy, and dead-letter processing job queues for background ingestion.
...
- Model provider strategy: local/open-source first (`LLM_PROVIDER=local`, `EMBEDDING_PROVIDER=local`) through `app/rag/model_providers.py`. Defaults use deterministic hashing embeddings (`LOCAL_EMBEDDING_RUNTIME=hashing`), the in-memory vector index (`VECTOR_INDEX_BACKEND=memory`), no reranker (`RERANKER_PROVIDER=none`), and the extractive answer generator (`LOCAL_LLM_RUNTIME=extractive`). `LOCAL_MODEL_PROFILE=local-default`, `host-ollama`, `compose-ollama`, or `vllm-gpu` applies packaged local profiles. Ollama embeddings and answer generation are available through profiles or individual runtime settings; this has been smoke-tested from Docker backend/worker to Mac-host Ollama with `nomic-embed-text:latest` and `llama3.1:8b`. pgvector and Qdrant indexing are available through `VECTOR_INDEX_BACKEND`; deterministic local keyword reranking and HTTP-backed local rerankers are available through the reranker boundary. Public OpenAI-compatible providers are implemented for embeddings and answer generation, but remain disabled by default and require `PUBLIC_LLM_ENABLED=true`, `PUBLIC_LLM_API_KEY`, and approved model names.
- Model status: `/api/v1/model-status` reports the active model profile, GPU profile label, embedding, answer, vector-index, and reranker runtime, model names, readiness, latency warning thresholds, and Ollama/Qdrant reachability when selected. Hashing/extractive/memory/default reranker settings report ready without network calls; Ollama runtimes check `/api/tags`; Qdrant checks the configured collection.
- Admin analytics: `/api/v1/analytics` reports tenant-scoped document, job, recent query/cache/latency, retrieval backend/reranker warning state, and audit-event rollups from PostgreSQL when persistence is enabled, falls back to in-memory document and query stores for local tests, embeds the retrieval evaluation quality summary, and accepts `action`/`resource_type` filters for recent audit events.
